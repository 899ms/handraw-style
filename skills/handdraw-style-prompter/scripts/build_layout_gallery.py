...
CATEGORIES = (("social-card", "社媒卡"), ("infographic", "信息图"), ("comic-storyboard", "漫画分镜"))


def gallery_html(layouts: list[dict[str, object]]) -> str:
    counts = {category: sum(layout["category"] == category for layout in layouts) for category, _ in CATEGORIES}
    visible_categories = [(category, label) for category, label in CATEGORIES if counts[category]]
    initial_category = visible_categories[0][0] if visible_categories else ""
    controls = "".join(
        f'<button class="filter" type="button" data-category="{category}">{label} <span>{counts[category]}</span></button>'
...
function openPreview(card){{activePrompt=card.dataset.prompt;preview.src=card.dataset.image;preview.alt=`${{card.dataset.id||''}} ${{card.dataset.name}}`;label.textContent=`${{card.querySelector('.layout-id').textContent}} · ${{card.dataset.name}}`;copyButton.textContent='复制排版提示词';dialog.showModal();}}
async function copyPrompt(){{try{{await navigator.clipboard.writeText(activePrompt)}}catch{{const area=document.createElement('textarea');area.value=activePrompt;area.style.position='fixed';area.style.opacity='0';document.body.append(area);area.select();document.execCommand('copy');area.remove();}}copyButton.textContent='已复制';}}
filters.forEach(button=>button.addEventListener('click',()=>setCategory(button.dataset.category)));cards.forEach(card=>card.addEventListener('click',()=>openPreview(card)));copyButton.addEventListener('click',copyPrompt);dialog.querySelector('.close').addEventListener('click',()=>dialog.close());dialog.addEventListener('click',event=>{{if(event.target===dialog)dialog.close()}});setCategory('{initial_category}');
</script></body></html>'''
