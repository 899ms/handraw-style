...
<dialog id="preview" aria-labelledby="dialog-label"><button class="close" type="button" aria-label="关闭放大预览">关闭 ×</button><img id="preview-image" alt=""><p class="dialog-label" id="dialog-label"></p><p class="dialog-tip">图片出于展示目的做了压缩，AI出的图字迹是很清晰的</p></dialog>
...
function showUpdateStatus(remoteVer){{updateStatus.hidden=false;updateStatus.className='update-status is-update';updateStatus.replaceChildren();const heading=document.createElement('strong'),message=document.createElement('span'),copyButton=document.createElement('button');heading.textContent='发现风格库更新'+(remoteVer?' (v'+remoteVer+')':'');message.textContent='请复制更新指令，然后丢给 Codex 进行更新。';copyButton.type='button';copyButton.textContent='复制更新指令';copyButton.addEventListener('click',async()=>{{try{{await navigator.clipboard.writeText(updateCommand);copyButton.textContent='已复制';}}catch{{copyButton.textContent='复制失败，请手动复制';}}}});updateStatus.append(heading,message,copyButton);}}
